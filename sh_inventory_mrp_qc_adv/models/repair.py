...
class QCMrpProduction(models.Model):
    ...
    quality_status = fields.Selection(
        [('pass', 'Pass'), ('fail', 'Fail'), ('pending', 'Pending'), ('partial', 'Partial')], string="Quality status")
    ...
    def action_approve(self):
        res = super(QCMrpProduction, self).action_approve()
        if self:
            counter = self.total_checks
            domain = [('picking_type_ids.id', '=', 22),
                      ('product_ids.id', '=', self.product_id.id), '|',
                      ('sh_team_id.user_ids.id', 'in', [self.env.uid]), ('sh_team_id', '=', False)]
            quality_points = self.env['sh.qc.point'].search(domain)
            if quality_points:
                self.has_qc = True
            for points in quality_points:
                domain = [('product_id', '=', self.product_id.id), ('production_id', '=', self.id),
                          ('sh_Qc_point_id', '=', points.id)]
                already_qc = self.env['sh.quality.check'].search(domain)
                latest_picking = self.env['stock.picking'].search([], order="id desc", limit=1)
                latest_picking_id = latest_picking.id if latest_picking else False
                if not already_qc:
                    counter += 1
                    qc_vals = {
                        ...
                        'counter': counter,
                        'picking_id': latest_picking_id,  # Set the new picking ID
                    }
    ...
    def check_other_round(self, counter):
        flag = False
        if self:
            domain = [('picking_type_ids.id', 'in', [22]),
                      ('product_ids.id', 'in', [self.product_id.id]), '|',
                      ('sh_team_id.user_ids.id', 'in', [self.env.uid]), ('sh_team_id', '=', False)]
            quality_points = self.env['sh.qc.point'].search(domain)
            for points in quality_points:
                domain = [('product_id', '=', self.product_id.id), ('production_id', '=', self.id),
    ...
    def _count_total_qc(self):
        for rec in self:
            rec.qc_count = len(rec.quality_check_line_lines) if rec.quality_check_line_lines else 0

    def _count_total_alerts(self):
        for rec in self or []:
            rec.qc_alert_count = len(rec.sh_quality_alert_ids) if rec else 0
