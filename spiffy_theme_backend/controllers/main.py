...
class BackendConfigration(http.Controller):
    ...
    def text_color_label_color(self, **kw):
        ...
        if 'options' in kw:
            if 'file_generator' and 'options' in kw['options']:
                ...
                uid = request.uid
                allowed_company_ids = [company_data['id']
                ...
                if not allowed_company_ids:
                    ...
                    allowed_company_ids = [int(str_id)
                                           for str_id in company_str.split(',')]
                report = request.env['account.report'].sudo().with_user(uid).with_context(
                    allowed_company_ids=allowed_company_ids).browse(options['report_id'])
                btn_report_data = report.dispatch_report_action(
                ...
                if type_ in ['qweb-pdf', 'qweb-text']:
                    ...
                    if docids:
                        docids = [int(i)
                                  for i in docids.split(',') if i.isdigit()]
                    report_data = report.sudo().with_context(
                        context)._render_qweb_pdf(reportname, docids, data=data)[0]

                    report_obj = request.env['ir.actions.report']
                    filereport = report_obj.with_context(context).sudo().search(
                        [('report_name', '=', reportname)], limit=1)
                ...
                if not import_compat and groupby:
                    ...
                    export_data = records.export_data(
    ...
    def show_user_todo_list(self, **kw):
        ...
        response = request.render(
            "spiffy_theme_backend.to_do_list_template", values)
...
class Session(WebSession):
    ...
    def authenticate(self, db, login, password, base_location=None):
        ...
        module_obj = request.env['ir.module.module'].sudo().search(
            [('name', '=', 'spiffy_theme_backend'), ('state', '=', 'installed')])
        if module_obj:
            if request.env.context.get('color_data'):
                ...
                view_obj = request.env['ir.ui.view'].sudo().search(
                    ['|', ('key', '=', color_data), ('key', '=', theme_color)])
                if view_obj:
...
class Home(WebHome):
    ...
    def web_client(self, s_action=None, **kw):
        ...
        if 'response' in kw:
            ...
            user = user_obj.search(user_obj._get_login_domain(
                color_user), order=user_obj._get_login_order(), limit=1)
